...
describe('findPath', () => {
  ...
  it('routes around walls (non-walkable cells)', () => {
    // Grid: start at (0,0), wall at (1,0), goal at (2,0)
    // Must go through (0,1) → (1,1) → (2,0) or similar diagonal route
    const cells = [
    ...
    // Ensure we don't walk through the wall
    expect(path.some((p: { x: number; y: number }) => p.x === 1 && p.y === 0)).toBe(false);
  });
  ...
  it('returns empty array when no path exists (walled off)', () => {
    // Island: (0,0) surrounded by walls, (2,2) unreachable
    const cells = [
