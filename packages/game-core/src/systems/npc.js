...
export function processTalkNpc(state, npcId, rng) {
    ...
    if (npc.role === 'informant') {
        ...
        const hasActiveOrReadyQuest = state.activeQuests.some(q => q.status === 'active' || q.status === 'ready_to_turn_in');
        ...
        const hasQuestFromThisGiver = state.activeQuests.some(q => q.giverNpcId === npcId && q.status === 'active');
