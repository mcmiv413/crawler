 ...
 */
export function checkRespawn(state, biome, rng) {
    if (state.run === null)
        return { state, events: [] };
    let events = [];
    let newState = state;
    ...
    const playerFov = new Set(Array.from(updatedFloor).filter(([, cell]) => cell.visibility === 'visible').map(([key]) => key));
    const candidateSpawns = Array.from(state.run.floor.cells)
        .filter(([key, cell]) => {
 ...
 */
function instantiateEnemy(template, position, depth, _rng) {
 ...
 */
export function respawnEnemiesOnPersistedFloor(floor, currentEnemies, originalEnemyCount, biome, depth, turnsSinceVisit, rng) {
    ...
    const playerFov = new Set(Array.from(visibleCells).filter(([, cell]) => cell.visibility === 'visible').map(([key]) => key));
    const candidateSpawns = Array.from(floor.cells)
        .filter(([key, cell]) => {
        if (cell.tile.walkable !== true)
            return false;
        const [x, y] = key.split(',').map(Number);
        if (x === undefined || y === undefined)
            return false;
        if (newEnemies.has(key))
            return false;
        if (playerFov.has(key))
            return false;
        // Spawn away from entrance
        if (chebyshevDistance({ x, y }, floor.entrance) < 5)
            return false;
        return true;
    })
        .map(([key]) => {
 ...
 */
export function simulatePersistedFloorTimeElapsed(storedFloor, turnsSinceVisit, biome, depth, rng) {
    const originalCount = storedFloor.originalEnemyCount ?? 0;
    // Respawn enemies based on time elapsed
    const respawnedEnemies = respawnEnemiesOnPersistedFloor(storedFloor.floor, storedFloor.enemies, originalCount, biome, depth, turnsSinceVisit, rng);
    // Run 10 turns of ambient behavior simulation so enemies aren't idle on re-entry
    const simulatedEnemies = preSimulateAmbientBehavior(new Map(respawnedEnemies), AMBIENT_PROFILES, 10, storedFloor.floor.seed);
    return {
        ...storedFloor,
...
//# sourceMappingURL=enemy-respawn.js.map