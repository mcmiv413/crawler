import type { GameState, NemesisRecord, EntityId, WeaponTemplate, WeaponType } from '@dungeon/contracts';
...
export function shouldPromoteToNemesis(
  state: GameState,
  enemy: EnemyInstance,
  floor: number,
  rng: SeededRNG,
): boolean {
  if (floor < NEMESIS_PROMOTION.minFloorForPromotion) return false;

  // First death ever always promotes — teaches the mechanic
  if (state.world.nemeses.length === 0) return true;

  const activeCount = state.world.nemeses.filter(n => n.isActive).length;
  if (activeCount >= NEMESIS_PROMOTION.maxActiveNemeses) return false;
...
export function promoteToNemesis(
  state: GameState,
  enemy: EnemyInstance,
  floor: number,
  rng: SeededRNG,
  ...
  const existingByTemplate = state.world.nemeses.filter(
    n => n.sourceTemplateId === enemy.templateId,
  );
  const rank = Math.min(3, existingByTemplate.length + 1) as 1 | 2 | 3;
  ...
  if (maxHealth < minHp) maxHealth = minHp;
  ...
  const equippedWeaponType = state.player.equipment.weapon !== null
    ? ((): WeaponType | null => {
        ...
        if (tpl !== undefined && tpl.itemClass === 'weapon') return (tpl as WeaponTemplate).weapon.weaponType;
        return null;
      })()
    : null;
...
export function slayNemesis(
  ...
  let blueprintToUnlock: string | null = null;
  ...
  const newState = {
    ...
    world: {
      ...
      nemeses: state.world.nemeses.map(n =>
        n.id === nemesisId
...
export function findNemesisByTemplateId(
  nemeses: readonly NemesisRecord[],
  templateId: string,
): NemesisRecord | undefined {
