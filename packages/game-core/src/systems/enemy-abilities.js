...
export const ENEMY_ABILITY_DEFINITIONS = new Map([
    [
        'crushing_blow',
        {
            id: 'crushing_blow',
 ...
 * Resolve the effects of an enemy using an ability.
 */
export function resolveEnemyAbility(abilityId, enemy, state, rng) {
    if (state.run === null)
        return { state, events: [] };
    const abilityDef = ENEMY_ABILITY_DEFINITIONS.get(abilityId);
    if (abilityDef === undefined)
        return { state, events: [] };
    let events = [];
    ...
    if (abilityDef.statusId !== undefined && abilityDef.statusDuration !== undefined && abilityDef.statusMagnitude !== undefined) {
        if (abilityDef.targetSelf === true) {
            ...
            const updatedPlayer = applyStatusToPlayer(newState.player, abilityDef.statusId, abilityDef.statusDuration, abilityDef.statusMagnitude, enemy.id);
...
//# sourceMappingURL=enemy-abilities.js.map