 ...
 */
export function scoreTiles(enemy, profile, state) {
    if (state.run === null)
        return new Map();
    ...
    const alliesOfSameType = new Map();
    const alliesOfOtherTypes = new Map();
    ...
    for (const [key, cell] of floor.cells.entries()) {
        ...
        if (prefs.sameSpeciesProximity !== undefined && prefs.sameSpeciesProximity > 0) {
            ...
            const nearbyAllies = Array.from(alliesOfSameType.values()).filter((allyPos) => chebyshevDistance(pos, allyPos) <= socialRadius).length;
        ...
        if (prefs.otherEnemyAvoidance !== undefined && prefs.otherEnemyAvoidance > 0) {
            const socialRadius = profile.socialRadius ?? 5;
            const nearbyOthers = Array.from(alliesOfOtherTypes.values()).filter((otherPos) => chebyshevDistance(pos, otherPos) <= socialRadius).length;
 ...
 */
export function analyzeSocialState(enemy, profile, state) {
    if (state.run === null) {
 ...
 */
export function shouldTransition(enemy, profile, rule, state, rng) {
    if (enemy.ambientState !== rule.from)
        return false;
 ...
 */
function pickBestTile(scores, walkableTiles, currentPos, rng, wanderIntensity) {
 ...
 */
export function decideAmbientAction(enemy, profile, state, rng) {
    if (state.run === null) {
        return {
 ...
 */
export function preSimulateAmbientBehavior(enemies, profiles, rounds, seed) {
...
//# sourceMappingURL=ambient-behavior-engine.js.map