...
export function createDamageDebugEvent(targetName, outcome, source) {
 ...
 */
export function applyDamageToPlayer(state, input) {
...
function findEnemyById(state, enemyId) {
    if (state.run === null)
        return undefined;
 ...
 * Handlers are responsible for emitting appropriate events.
 */
export function applyDamageToEnemy(state, enemyId, input) {
    if (state.run === null) {
