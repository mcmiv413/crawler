...
const ARMOR_EQUIP_SLOTS = ['chest', 'head', 'gloves', 'boots', 'ring1', 'ring2'];
/**
...
function findEquipmentSlot(equipment, itemId) {
    if (equipment.weapon === itemId)
        return 'weapon';
    if (equipment.chest === itemId)
        return 'chest';
    if (equipment.head === itemId)
        return 'head';
    if (equipment.gloves === itemId)
        return 'gloves';
    if (equipment.boots === itemId)
        return 'boots';
    if (equipment.ring1 === itemId)
        return 'ring1';
    if (equipment.ring2 === itemId)
        return 'ring2';
 ...
 */
function withRecalculatedStats(state, newEquipment) {
    const newStats = calculateEquippedStats(state.player.baseStats, state.player.stats.health, newEquipment, state.itemRegistry.items);
    return {
        ...state.player,
 ...
 */
export function calculateEquippedStats(baseStats, currentHealth, equipment, registry) {
    ...
    const resistances = { ...baseStats.resistances };
    ...
    if (Object.keys(resistances).length > 0) {
        return { ...result, resistances: resistances };
    }
    return result;
}
/**
 ...
 */
function unlockBlueprints(state, blueprintIds) {
...
export function equipItem(state, itemId) {
    ...
    if (template.itemClass === 'weapon') {
        ...
        const enchIds = armor.enchantments.filter((e) => e !== null);
        if (enchIds.length > 0) {
 ...
 */
export function swapWeaponSets(state) {
    const events = [];
    const newEquipment = {
