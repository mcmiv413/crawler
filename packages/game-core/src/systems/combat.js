 ...
 */
export function resolveAttack(ctx, rng, onHitStatus, onHitChance, config) {
    const combatConfig = config?.combat ?? COMBAT;
    // Step 3: Hit resolution
    const hitChance = calculateHitChance(combatConfig.baseHitChance, ctx.attackerAccuracy, ctx.defenderEvasion, combatConfig.minHitChance, combatConfig.maxHitChance);
    ...
    if (hit !== true) {
        ...
        const hitChanceWithoutEvasion = calculateHitChance(combatConfig.baseHitChance, ctx.attackerAccuracy, 0, // No evasion
        combatConfig.minHitChance, combatConfig.maxHitChance);
    ...
    if (ctx.weaponDamageProfile !== undefined && ctx.weaponBaseDamage !== undefined) {
        ...
        const band = getDamageBand(ctx.weaponBaseDamage, ctx.weaponDamageProfile);
    ...
    const statusesApplied = (onHitStatus !== undefined && onHitChance !== undefined && rng.chance(onHitChance))
        ? [onHitStatus]
        : [];
