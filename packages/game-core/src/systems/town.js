...
export function processEnchantArmor(state, equipSlot, enchantmentId) {
    ...
    const newEnchantments = [...armor.enchantments];
    ...
    const newStats = calculateEquippedStats(state.player.baseStats, state.player.stats.health, newEquipment, newRegistry);
    const events = [{
...
export function processTownAction(state, action, targetId, itemId, rng) {
    switch (action) {
        case 'rest':
...
function processShopBuy(state, itemId) {
    if (itemId === undefined)
        return { state, events: [] };
    ...
    const newShopItems = state.world.shop.items.map(i => i.itemId === itemId ? { ...i, stock: i.stock - 1 } : i);
    let updatedState = {
        ...state,
        player: {
            ...state.player,
...
function processShopSell(state, itemEntityId) {
...
function processShopUndo(state) {
