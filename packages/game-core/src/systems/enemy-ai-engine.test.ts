...
/**
 * Minimal archetype stubs for unit-level testing — no @dungeon/content import needed.
 * Cross-reference coverage (every enemy template references a real archetype) lives in
 * tests/contracts/archetype-coverage.contract.test.ts.
 */
const STUB_AGGRESSIVE_MELEE: ArchetypeDefinition = {
  id: 'aggressive_melee',
  ...
  actionSelection: [
    { condition: 'playerAdjacent', actions: ['attack', 'ability'], weight: 4 },
    { condition: 'playerRange2to5', actions: ['move'], weight: 2 },
    { condition: 'hpBelowThreshold(0.25)', actions: ['attack', 'ability'], weight: 5 },
  ],
  panicThresholds: [
...
const STUB_SKITTISH_RANGED: ArchetypeDefinition = {
  id: 'skittish_ranged',
  ...
  actionSelection: [
    { condition: 'playerAdjacent', actions: ['move'], weight: 4 },
    { condition: 'playerRange2to5', actions: ['wait'], weight: 3 },
    { condition: 'playerRange6Plus', actions: ['move'], weight: 2 },
    { condition: 'hpBelowThreshold(0.30)', actions: ['move'], weight: 5 },
  ],
  panicThresholds: [
...
const STUB_CAUTIOUS_DEFENSIVE: ArchetypeDefinition = {
  ...
  actionSelection: [
    { condition: 'playerAdjacent', actions: ['attack'], weight: 2 },
    { condition: 'hpBelowThreshold(0.40)', actions: ['move'], weight: 3 },
    { condition: 'hpAboveThreshold(0.60)', actions: ['ability', 'wait'], weight: 2 },
  ],
  panicThresholds: [
...
const STUB_HAZARD_CREATOR: ArchetypeDefinition = {
  id: 'hazard_creator',
  name: 'Hazard Creator',
  ...
  actionSelection: [
    { condition: 'playerRange2to4', actions: ['ability'], weight: 3 },
    { condition: 'playerAdjacent', actions: ['move'], weight: 3 },
  ],
  abilityPreferences: [
...
const STUB_AMBUSHER: ArchetypeDefinition = {
  id: 'ambusher',
  name: 'Ambusher',
  description: 'Strikes from hiding, retreats after attacking.',
  targetSelection: [
    { factor: 'proximity', weight: 1, direction: 'minimize' },
  ],
  positioning: [
    { factor: 'rangeToPlayer', targetDistance: 2, weight: 2 },
  ],
  actionSelection: [
    { condition: 'playerAdjacent', actions: ['attack', 'ability'], weight: 4 },
    { condition: 'playerRange2to5', actions: ['move'], weight: 2 },
  ...
  panicThresholds: [
    { hpPercent: 20, behavior: 'retreat' },
  ],
  abilityPreferences: [
    { abilityId: 'backstab', weight: 3, usageHp: 'anytime' },
  ],
};

/** Local stub map — mirrors ARCHETYPES from @dungeon/content without the live import */
const STUB_ARCHETYPES: ReadonlyMap<string, ArchetypeDefinition> = new Map([
  ['aggressive_melee', STUB_AGGRESSIVE_MELEE],
  ['skittish_ranged', STUB_SKITTISH_RANGED],
  ['cautious_defensive', STUB_CAUTIOUS_DEFENSIVE],
  ['hazard_creator', STUB_HAZARD_CREATOR],
  ['ambusher', STUB_AMBUSHER],
]);
...
describe('Enemy AI Scoring Engine', () => {
  describe('scoreEnemyActions', () => {
    it('returns valid trace with candidates', () => {
      ...
      const archetype = STUB_ARCHETYPES.get('aggressive_melee');
    ...
    it('skittish_ranged at distance 1 prefers move', () => {
      ...
      const archetype = STUB_ARCHETYPES.get('skittish_ranged');
      const rng = new SeededRNG(42);
  ...
  describe('archetype behavior patterns', () => {
    it('all archetypes produce valid decisions', () => {
      ...
      for (const [archetypeId, archetype] of STUB_ARCHETYPES) {
        const enemy = createTestEnemy({
          position: { x: 2, y: 0 },
  ...
  describe('edge cases', () => {
    ...
    it('handles multiple candidates with same score', () => {
      const state = createTestGameStateInCombat();
      const enemy = createTestEnemy({ position: { x: 5, y: 5 }, isAlerted: true });
      const archetype = STUB_ARCHETYPES.get('cautious_defensive');
      const rng = new SeededRNG(42);
  ...
  describe('Ability cooldown filtering and tie-breaking', () => {
    it('filters out abilities on cooldown from candidates', () => {
      ...
      const archetype = STUB_ARCHETYPES.get('hazard_creator');
      const rng = new SeededRNG(42);
