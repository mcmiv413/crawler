 ...
 */
export function evaluateQuestProgress(quest, state) {
 ...
 */
function updateQuestProgress(quest, state) {
    ...
    switch (objective.type) {
        ...
        case 'reach_floor': {
            ...
            const targetCount = objective.targetCount;
 ...
 */
function isObjectiveSatisfied(quest) {
 ...
 */
export function redeemQuest(state, quest) {
    ...
    const rewardAmount = quest.reward.amount;
    const updatedState = {
        ...state,
        activeQuests: state.activeQuests.map(q => q.id === quest.id ? { ...q, status: 'rewarded' } : q),
        player: {
            ...state.player,
