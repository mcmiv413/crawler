 ...
 */
const MOVEMENT_BEHAVIOR_REGISTRY = [
    {
        id: 'wall_stalker',
        name: 'Wall Stalker',
 ...
 */
export function getBehaviorInfo(id) {
 ...
 */
export function getWalkableNeighbors(position, state) {
    if (state.run === null)
        return [];
    const mutableNeighbors = [];
 ...
 */
export function decideMovementByBehavior(enemy, state) {
 ...
 */
export function scoreTilesForBehavior(enemy, availableTiles, state, behaviorId) {
 ...
 */
function scoreWallStalker(_enemy, availableTiles, state) {
    if (state.run === null)
        return availableTiles.map(tile => ({ tile, score: 0 }));
 ...
 */
function scoreRearlineAnchor(_enemy, availableTiles, state) {
 ...
 */
function chokepointHolder(_enemy, availableTiles, state) {
    if (state.run === null)
        return availableTiles.map(tile => ({ tile, score: 0 }));
    const playerPos = state.player.position;
    const floor = state.run.floor;
 ...
 */
function scoreChokepointHolder(enemy, availableTiles, state) {
 ...
 */
function scoreAmbushIdle(_enemy, availableTiles, state) {
    if (state.run === null)
        return availableTiles.map(tile => ({ tile, score: 0 }));
    const playerPos = state.player.position;
    const floor = state.run.floor;
 ...
 */
function scoreDefaultApproach(_enemy, availableTiles, state) {
    const playerPos = state.player.position;
    return availableTiles.map(tile => {
...
//# sourceMappingURL=movement-behaviors.js.map