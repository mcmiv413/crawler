...
export function validateMove(state, direction) {
...
export function isWalkable(state, pos) {
    if (state.run === null)
        return false;
    ...
    for (const enemy of state.run.enemies.values()) {
        if (positionsEqual(enemy.position, pos))
            return false;
    }
    // Check player position
    if (positionsEqual(state.player.position, pos))
        return false;
    return true;
}
