...
export function populateFloor(
  ...
  for (let i = 0; i < enemyCount && i < shuffled.length; i++) {
    ...
    if (worldMods?.tierUpgradeChance !== undefined && rng.next() < worldMods.tierUpgradeChance) {
      ...
      const upgradedCandidates = biomePool
        .filter((t): t is EnemyTemplate => t.tier > template!.tier);
  ...
  if (worldMods?.nemesesToSpawn && worldMods.nemesesToSpawn.length > 0) {
    for (const nemesis of worldMods.nemesesToSpawn) {
      ...
      const nemesisInstance: EnemyInstance = {
        ...nemesisEnemy,
        name: nemesis.name,
        stats: nemesis.stats,
...
function pickEnemy(biome: BiomeDefinition, rng: SeededRNG, depth: number, worldMods?: WorldModifiers): EnemyTemplate | null {
  const biomePool = ENEMIES_BY_BIOME.get(biome.biomeId) ?? [];
  ...
  for (const templateId of extendedPool) {
    ...
    if (!template || template.tier > maxTier) continue;
    ...
    if (preferred.has(templateId)) weight *= 3;
