...
import { ECONOMY, CONSUMABLES, WEAPONS, ARMOR, INITIAL_FACTIONS, rustySword, ironMace, shortBow, handAxe, flameDagger, venomBlade, warBow, stoneHammer, frostAxe, ironSword, leatherVest, leatherCap, leatherGloves, leatherBoots, copperRing, chainShirt, ironHelm, chainGauntlets } from '@dungeon/content';
/** Create initial world state for a new game */
export function createInitialWorldState(rng) {
    return {
...
function createInitialTownState() {
    return {
        prosperity: 50,
        fear: 10,
        corruption: 0,
...
function createInitialShop(rng) {
...
export function randomizeShop(rng) {
    const allItems = [...WEAPONS, ...ARMOR];
    // Randomize weapon selection: pick 3-4 random weapons from the curated list
    const shuffledWeapons = rng.shuffle(SHOP_WEAPON_IDS);
    const selectedWeaponCount = 3 + rng.int(0, 1); // 3 or 4
