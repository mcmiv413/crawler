 ...
 */
export function processEnemyKill(state, enemy, enemyPosKey, rng) {
    let events = [];
    let newState = state;
    ...
    if (goldFromLoot > 0)
        newState = updateRunMetrics(newState, { goldEarned: goldFromLoot });
    ...
    if (enemy.nemesisId !== undefined) {
        ...
        const activeNemesis = newState.world.nemeses.find(n => n.isActive && n.sourceTemplateId === enemy.templateId);
...
export function handleAttack(state, targetId, rng) {
    if (state.run === null)
    ...
    if (state.player.equipment.weapon !== null) {
        ...
        if (weaponTemplate !== undefined && weaponTemplate.itemClass === 'weapon') {
            const weapon = weaponTemplate.weapon;
    ...
    if (result.hit === true) {
        ...
        for (const statusId of result.statusesApplied) {
            ...
            const duration = 'defaultDuration' in defaults ? defaults.defaultDuration : 3;
...
export function handleUseAbility(state, abilityId, rng, targetId, direction) {
    if (state.run === null)
        return { state, events: [], runEnded: false };
    if (canUseAbility(state, abilityId) !== true)
        return { state, events: [], runEnded: false };
    ...
    if (Array.isArray(def.requiresWeaponTypes) && def.requiresWeaponTypes.length > 0) {
        ...
        if (!equippedType || !def.requiresWeaponTypes.includes(equippedType))
            return { state, events: [], runEnded: false };
    }
    // Route directional abilities to their handlers
    if (abilityId === daggerDisarm.id && direction) {
        return handleDisarmTrap(state, direction, rng);
    }
    if (abilityId === daggerSetTrap.id && direction && targetId) {
        return handleSetTrap(state, direction, targetId, rng);
    }
    ...
    let resultEvents = [...abilityResult.events];
