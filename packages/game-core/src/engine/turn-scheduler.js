...
import { decideEnemyAction } from '../systems/enemy-ai.js';
...
export function processEnemyTurns(state, rng, playerSpeed) {
    if (state.run === null)
        return { state, events: [] };
    ...
    for (const enemy of enemies) {
        ...
        while (true) {
            if (currentState.player.stats.health <= 0)
                break;
            ...
            if (playerSpeed !== undefined) {
                ...
                if (accumulator < 1)
                    break; // Enemy skips this turn (player is relatively faster)
            }
            else {
                if (actionsThisTurn >= 1)
                    break; // Without speed system, each enemy acts once
            }
            ...
            if (currentEnemy === undefined || currentEnemy.id !== enemy.id)
                break; // Enemy dead or moved away
            // Alert check
            ...
            if (currentEnemy.isAlerted !== true) {
                ...
                if (dist <= 5) {
                    ...
                    if (updatedEnemy.nemesisId !== undefined) {
                        ...
                        const updatedNemeses = currentState.world.nemeses.map(n => n.id === updatedEnemy.nemesisId
            ...
            if (updatedEnemy.statuses.some(s => s.id === 'stun'))
                break;
            // Decide action based on alert state
            let action;
            let ambientStateChangeEvent = null;
...
export function tickEnemyCooldowns(state) {
    ...
    for (const [key, enemy] of newEnemies) {
        if (enemy.abilityCooldowns === undefined || Object.keys(enemy.abilityCooldowns).length === 0)
            continue;
...
function executeEnemyAction(action, enemy, state, rng) {
    if (state.run === null)
        return { state, events: [] };
    switch (action.type) {
        case 'move': {
            ...
            if (hazardAtPos !== undefined) {
                ...
                if (hazardTemplate !== undefined && hazardTemplate.isHazard === true) {
                    ...
                    const damageResult = applyDamageToEnemy(newState, movedEnemy.id, {
                        amount: trapDamage,
                        damageType,
                        source: 'trap',
        ...
        case 'attack': {
            ...
            if (weaponRange > 1 || minRange > 0) {
                ...
                effectiveAccuracy = applyRangeAccuracyPenalty(effectiveAccuracy, dist, minRange, COMBAT.rangedAccuracyDropPerTile);
            }
            ...
            const expectedHitChance = Math.max(COMBAT.minHitChance, Math.min(COMBAT.maxHitChance, COMBAT.baseHitChance + effectiveAccuracy - playerEvasion));
