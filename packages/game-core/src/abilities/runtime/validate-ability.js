 ...
 */
export function validateRequirements(context, requirements) {
...
function validateSingleRequirement(context, req) {
    switch (req.kind) {
        case 'weapon_type': {
        ...
        case 'target_in_melee_range': {
            ...
            const dist = Math.max(Math.abs(context.player.position.x - context.target.instance.position.x), Math.abs(context.player.position.y - context.target.instance.position.y));
            if (dist > 1) {
        ...
        default:
            ...
            throw new Error(`Unknown requirement kind: ${req.kind}`);
    }
}
//# sourceMappingURL=validate-ability.js.map