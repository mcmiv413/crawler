...
9. For deterministic proof planning, install the released `proofctl 0.4.2`, run `proofctl plan --repository PATH --base BASE_SHA --head HEAD_SHA`, copy the selected obligations into the implementation plan, and do not choose a smaller proof set.
